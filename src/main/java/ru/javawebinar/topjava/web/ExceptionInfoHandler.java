...
public class ExceptionInfoHandler {
    ...
    public ErrorInfo illegalRequestDataError(HttpServletRequest req, Exception e, BindingResult result) {
        ...
        if (result != null && result.hasErrors()) {
            cause = ValidationUtil.getErrorResponse(result);
        }
        return logAndGetErrorInfo(req, e, false, VALIDATION_ERROR, cause);
    }
    ...
    public ResponseEntity<String> handleMethodArgumentNotValidException(MethodArgumentNotValidException error ) {
        return new ResponseEntity<>(error.getBindingResult().toString(), HttpStatus.UNPROCESSABLE_ENTITY);
    }

    private static ErrorInfo logAndGetErrorInfo(HttpServletRequest req, Exception e, boolean logException, ErrorType errorType, String cause) {
        ...
        return new ErrorInfo(req.getRequestURL(), errorType, cause == null ? rootCause.toString() : cause);
    }
}